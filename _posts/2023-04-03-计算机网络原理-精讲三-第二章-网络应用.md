...
1. URL 地址主要两个部分组成： 存放对象的<font color='red' style='background-color:' size=''>主机域名（ 或 IP 地址）</font> 和 <font color='red' style='background-color:' size=''>对象的路径名称</font>。
   例：` https://www.abc.edu.cn/cs/index.html `
...
1. 1904综合题 13分：
   ...
   设某网页的URL为https://www.abc.com/index.html，文件index.html引用了 8个小图像。在域名解析的过程中，无等待的一次DNS解析请求与响应时间记为RTTd,HTTP请求传输Web对象过程的一次往返时间记为RTTh。试给出：
